...
7. **同族题演示行不得复用**：同一规律族（如叠加消去）的演示格形状组合不得与其它题逐格雷同（唯一性以「整题 cells 签名」为准，行级重叠在后续批次顺带优化，不作为本期硬约束）
...
npx jest src/modules/iq
npx tsc --noEmit
git add src/modules/iq/data/verbalItems.ts src/modules/iq/services/itemBank.test.ts
...
npx jest src/modules/iq
npx tsc --noEmit
git add scripts/gen-speed-rows.js src/modules/iq/data/memorySequences.ts src/modules/iq/data/speedTask.ts src/modules/iq/data/practiceItems.ts src/modules/iq/services/itemBank.test.ts
...
（评审修订追加（随修复提交落地）：① `scoring.ts` 补常量与依据注释——`INTERVAL_HALF_WIDTH = 7.5`（设计标定不确定度 ≈ ±0.5 SD）、`WEIGHTS` 与轻量星级阈值加「设计标定」说明，**计分行为不变**；② `scoring.test.ts` 补 3 组用例——`computeReport` 分发（pro/light 各断言 kind）、折算表不变量（四档四维：锚点 ≥5 点、x 严格升序、y 单调不减、末点指数 ≥125）、轻量星级档位（两题全对且净 0 → 4 星；一对一错且净 24 → 3 星；一对一错且净 6 → 2 星——避免浮点等值边界）；③ percentile 断言收紧为精确值（`toBe(63.1)` / `toBe(0.1)`）。）
...
（评审修订追加：T6 落地时 `paperBuilder.test.ts` 需补一条一致性用例——「折算表记忆封顶 = 各档组卷记忆范围上限之和」：对每档断言 `conversionTables[band].memory` 末点 x === `MEMORY_RANGES[band].forward[1] + MEMORY_RANGES[band].backward[1]`（为此 `MEMORY_RANGES` 需从 paperBuilder 导出）。）
...
（评审修订追加（随修复提交落地）：**存储 API 幂等化**——`recordChoice(itemId, optionIndex, elapsedMs)`、`recordMemory(trialId, input, elapsedMs)`，id 与当前 step 不符即 no-op（防 UI 双击重复提交污染下一题/误触天花板）；`finishSpeed` 改为**先同步进入完成态、再后台落盘**；`practiceUnit` 与天花板扫描补数据契约注释。对应测试调用点同步改为携带 id（`recordChoice(item.id, i, ms)` / `recordMemory(trial.id, input, ms)`），并新增 3 用例：非法调用 no-op、末段天花板落到 speed-ready、存储抛错仍产出结果（light）。）
