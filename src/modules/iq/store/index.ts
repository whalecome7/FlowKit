...
/** 练习题目按维度索引（step.kind === 'practice' 时用）。
 *  重载保证返回类型精确：section 必须传字面量，直接传 Dimension 会编译失败（'speed' 无练习）。
 *  fluid 的 Math.min 为防御数据脱节（practiceCounts 超出 PRACTICE_MATRIX 长度时取最后一题）。 */
...
interface IqState {
  ...
  /** 练习完成，推进下一步 */
  finishPractice: () => void;
  /** 正式选择题作答（图形/言语）：须携带当前题目 id（幂等防重复提交），自动进入下一题 */
  recordChoice: (itemId: string, optionIndex: number, elapsedMs: number) => void;
  /** 数字广度作答：须携带当前试次 id（幂等防重复提交），含天花板判定（同模式同长度两试皆错则跳过该模式剩余） */
  recordMemory: (trialId: string, input: number[], elapsedMs: number) => void;
  /** 速配说明页 → 开始作答 */
  beginSpeed: () => void;
  /** 速配结束：产出报告、写存储、进入完成态 */
...
export const useIqStore = create<IqState>((set, get) => ({
  ...
  recordChoice(itemId, optionIndex, elapsedMs) {
    const { session } = get();
    if (!session) return;
    const step = session.steps[session.stepIndex];
  ...
  recordMemory(trialId, input, elapsedMs) {
    ...
    if (!isMemoryPass(trial, input)) {
      ...
      // （扫描依赖同模式试次在 steps 中连续——由 buildSteps 编排保证；改组卷顺序需同步复审此处）
